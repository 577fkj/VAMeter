...
static void _pm_slide_appear(lv_pm_anima_data *anima_data)
{
    ...
    lv_anim_set_var(&appear_anima, anima_data->pm_page->inst->page);
...
static void _pm_slide_disAppear(lv_pm_anima_data *anima_data)
{
    ...
    lv_anim_set_var(&disAppear_anima, anima_data->pm_page->inst->page);
...
static void _pm_popup_appear(lv_pm_anima_data *anima_data)
{
    ...
    if (anima_data->pm_page->_back)
    {
        ...
        lv_obj_set_style_radius(anima_data->pm_page->inst->page, 0, LV_STATE_DEFAULT);
    }
    else
    {
        lv_anim_set_values(&appear_anima, height, POPUP_TOP_HEIGHT);
        lv_obj_set_style_radius(anima_data->pm_page->inst->page, 10, LV_STATE_DEFAULT);
    }
